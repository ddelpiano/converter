...
login = json.load(open('./data/local_neo4j.json', 'r'))
...
reply = confirm_prompt("Is this the first you are running the server? [y/n]")

if reply == "y":
    driver = GraphDatabase.driver(db_uri, auth=('neo4j', db_pwd), encrypted=False)
    # initialise local neo4j instance
    init_graph = '''CALL n10s.graphconfig.init();'''
    init_constraint = '''CREATE CONSTRAINT n10s_unique_uri FOR (r:Resource) REQUIRE r.uri IS UNIQUE;'''
    driver.execute_query(init_graph)
    driver.execute_query(init_constraint)
    driver.close()

# Populate Neo4j with .ttl
